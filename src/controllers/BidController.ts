...
export default class BidController {
    ...
    private youtube: YoutubeController | undefined
    ...
    static init = async (id: string, fileName: string, url: string) => {
        ...
        if(!BidController.shared.youtube){
            BidController.shared.youtube = await YoutubeController.init()
        }

        if(BidController.shared.youtube.url !== url) {
            await BidController.shared.youtube?.goto(url)
        }
    ...
    startTimer = (index: number) =>{
        ...
        this.timer = setInterval(async ()=>{
            ...
            const chatList = await this.youtube.getChat()
